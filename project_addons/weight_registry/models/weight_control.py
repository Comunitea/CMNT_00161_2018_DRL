...
REGISTRY_TYPE = [('incoming', 'Incoming'), ('outgoing', 'Outgoing'), ('production','Production'), ('internal', 'Internal'), ('wash', 'Wash'), ('other', 'Other')]
...
class WeightRegistry(models.Model):
    ...
    fill = fields.Boolean('Fill', compute='_compute_worked_hours', store=True, readonly=True)
    ...
    net = fields.Integer('Net weight', compute='_compute_worked_hours', store=True, readonly=True)
    partner_id = fields.Many2one('res.partner', 'Measured by', domain=[('driver', '=', True)])
    ...
    def apply_net_to_qty_done(self):
        ...
        for w_r in self.filtered(lambda x:x.check_out):
            ...
            if any(x.state=='done' for x in w_r.mapped('move_line_ids')):
                raise exceptions.ValidationError (_('Move {} is done.'.format(w_r.mapped('move_line_ids').mapped('display_name'))))
    ...
    @api.depends('check_in', 'check_out','check_in_weight', 'check_out_weight')
    def _compute_worked_hours(self):
        for registry in self:
            if registry.check_out:
                delta = datetime.strptime(registry.check_out, DEFAULT_SERVER_DATETIME_FORMAT) - datetime.strptime(
                    registry.check_in, DEFAULT_SERVER_DATETIME_FORMAT)
                registry.worked_hours = delta.total_seconds() / 3600.0

                registry.net = abs(registry.check_in_weight- registry.check_out_weight)
    ...
    def _check_validity(self):
        ...
        for registry in self:
            ...
            if last_reg_before_check_in and last_reg_before_check_in.check_out and last_reg_before_check_in.check_out > registry.check_in:
