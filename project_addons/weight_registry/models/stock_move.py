...
class StockMove(models.Model):
    ...
    weight_registry_id = fields.Many2one(
        'weight.registry', 'Registro de pesada')
    ...
    def _action_done(self):
        if self.filtered(
                lambda x: x.picking_type_id.need_weight_registry and not
                x.weight_registry_id and not x.weight_registry_id.check_out):
            raise ValidationError(
                _('You have moves with weight_registry_id not set or without check out'))
    ...
    def _prepare_move_line_vals(self, quantity=None, reserved_quant=None):
        ...
        vals.update({
            'weight_registry_id':
            self._context.get('weight_registry_id', False)})
